...
public class LightVisibilityEnemy : BaseEnemy
{
    ...
    {
        //viewLightObjectを安全に削除
        DestroySafe(ref viewLightObject);

        //playerLightを安全に削除
        DestroySafe(ref playerLight);

        //objectLightを安全に削除
        DestroySafe(ref objectLight);
    }
    ...
    protected override async void Update() 
    {
        ...
        {
            // 敵の目から「光が当たった点」への方向と距離
            Vector3 targetDir = playerlightHit.point - viewLightObject.position;
            float distanceToHit = targetDir.magnitude;
            Vector3 headToPlayerLightHitDir = targetDir.normalized;

            // --- 追加：視野角チェック (例: 前方60度以内なら検知) ---
            float angle = Vector3.Angle(viewLightObject.forward, headToPlayerLightHitDir);
            ...
            // 視野角内(例: 60度) かつ 距離が一定以内(例: 15m) の場合のみ反応
            if (angle < 60f && distanceToHit < 15f) 
            {
                //フラッシュライトからのRaycastが当たったオブジェクトに敵のRaycastが当たった場合
                if (Physics.Raycast(viewLightObject.position, headToPlayerLightHitDir, out seeTheLightHit, distanceToHit + 2.5f))
                {
        ...
        if (objectLight != null) 
        {
            ...
            if (Physics.Raycast(objectLight.position, objectLight.transform.forward, out objectlightHit, Mathf.Infinity) && !isViewPlayerLight)
            {
                // 敵の目から「光が当たった点」への方向と距離
                Vector3 targetDir = objectlightHit.point - viewLightObject.position;
                float distanceToHit = targetDir.magnitude;
                Vector3 headToObjectLightHitDir = targetDir.normalized;

                // --- 追加：視野角チェック (例: 前方60度以内なら検知) ---
                float angle = Vector3.Angle(viewLightObject.forward, headToObjectLightHitDir);
                ...
                Debug.DrawRay(objectLight.position, objectLight.transform.forward * 10, Color.yellow, 0.1f);

                // 視野角内(例: 60度) かつ 距離が一定以内(例: 15m) の場合のみ反応
                if (angle < 60f && distanceToHit < 15f) 
                {
                    //オブジェクトのライトからのRaycastが当たったオブジェクトに敵のRaycastが当たった場合
                    if (Physics.Raycast(viewLightObject.position, headToObjectLightHitDir, out seeTheLightHit, distanceToHit + 0.5f))
                    {

                        //オブジェクトのライトと視線が同じ場所に当たった場合（距離が近い場合も含む）
                        //if (lightHit.point == seeTheLightHit.point)に近い考え方である
                        if (Vector3.Distance(objectlightHit.point, seeTheLightHit.point) < 0.2f)
                        {
