...
public class MusicController : MonoBehaviour
{
    ...
    /// BGM用audioSourceのリスト
    /// </summary>
    private List<AudioSource> audioSourceBGMList = new();
    //TODO:1個目のリストにStageBGM,2個目のリストにChasePlayerBGMを格納した後、それぞれを使い分けるようにする

    /// <summary>
    /// BGM用audioSourceのリスト個数上限
    /// </summary>
    private const int audioSourceBGMListCapacity = 2;
    ...
    private void OnDestroy()
    {
        ...
        //AudioMixerGroupBGMが存在する場合
        if (audioMixerGroupBGM != null) 
        { 
            //audioMixerGroupBGMをnullにする
            audioMixerGroupBGM = null;
        }

        //AudioMixerGroupSEが存在する場合
        if (audioMixerGroupSE != null) 
        { 
            //audioMixerGroupSEをnullにする
